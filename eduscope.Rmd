...
data_simul_college_sub_carte <- data_simul_college %>% filter(acad == input$acad_id)

filo_sub_carte <- filo %>% filter(IRIS %in% unique(data_simul_college$IRIS))

dep_shp_sub_carte <- dep_shp %>% filter(code_insee %in% unique(data_simul_college$dep))

iris_shp_sub_carte <- iris_shp %>% filter(CODE_IRIS %in% filo_sub_carte$CODE_IRIS)

acad_shp_bbox <- acad_shp %>%
  filter(str_detect(.$name, input$acad_id)) %>%
  st_bbox()
  
leafletProxy("carto_etab") %>%
  map.fitBounds(acad_shp_bbox$xmin, acad_shp_bbox$ymin,
                acad_shp_bbox$xmax, acad_shp_bbox$ymax)
  })
